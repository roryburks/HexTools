...
infix fun RayRect.intersectsPrecise(obj: CollisionObject): Double? = when (obj) {
    ...
    is CollisionMultiObj -> obj.objs.asSequence().mapNotNull { this intersectsPrecise it }.minOrNull()
}
...
object RayRectCollision {
    ...
    fun withArc(rayRect: RayRect, arc: Arc): Double? {
        ...
        possibles.addAll(
            ArcCollision.arcSegmentWithLineSegment(arc, rayRect.side1).asSequence().map { roll(rayRect.side1, it) })
        possibles.addAll(
            ArcCollision.arcSegmentWithLineSegment(arc, rayRect.side2).asSequence().map { roll(rayRect.side2, it) })
