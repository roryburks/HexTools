...
class CollisionPolygon( val polygon: IPolygon) : CollisionObject() {
    override val bounds: Rect by lazy {
        ...
        RectD.FromEndpoints(
            p.map { it.x }.minOrNull() ?: 0.0,
            p.map { it.y }.minOrNull() ?: 0.0,
            p.map { it.x }.maxOrNull() ?: 0.0,
            p.map { it.y }.maxOrNull() ?: 0.0
        )
    }
...
class CollisionMultiObj(val objs: List<CollisionObject>) : CollisionObject() {
    override val bounds: Rect by lazy { RectD.FromEndpoints(
        objs.asSequence().map { it.bounds.x1 }.minOrNull() ?: 0.0,
        objs.asSequence().map { it.bounds.y1 }.minOrNull() ?: 0.0,
        objs.asSequence().map { it.bounds.x2 }.maxOrNull() ?: 0.0,
        objs.asSequence().map { it.bounds.y2 }.maxOrNull() ?: 0.0
    ) }
